...
CREATE TABLE IF NOT EXISTS Refraktion (RefraktionID int NOT NULL, MitarbeiterID int, ArztID int, Sph float, Cyl float, Ach int, Adds float, Pris float, Bas int, Visus float, PRIMARY KEY (RefraktionID), FOREIGN KEY (MitarbeiterNr) REFERENCES Mitarbeiter(MitarbeiterNr), FOREIGN KEY (ArztNr) REFERENCES Arzt(ArztNr));
...
CREATE TABLE IF NOT EXISTS Glaeser (ArtikelID int NOT NULL, Werte varchar(254), PRIMARY KEY (ArtikelNr), FOREIGN KEY (Werte) REFERENCES Refraktion(RefraktionID));

CREATE TABLE IF NOT EXISTS Fassungen (ArtikelID int NOT NULL, WerteID int, PRIMARY KEY (ArtikelNr));

CREATE TABLE IF NOT EXISTS Kontaktlinsen (ArtikelID int NOT NULL, WerteID int, PRIMARY KEY (ArtikelNr));
